...
export function Post({ post, onReply, onViewReplies, onDeleted, onReposted, isReply = false }: PostProps) {
  ...
  return (
    <div 
      className={`bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-4 transition-shadow hover:shadow-sm ${isReply ? 'ml-6 border-l-4 border-l-gray-200 dark:border-l-gray-800' : ''}`}
    ...
    >
      <div className="flex items-start justify-between mb-2">
        ...
        <div className="text-xs font-mono text-gray-300 dark:text-gray-500 flex items-center gap-3">
          {typeof post.repost_count === 'number' && (
            <span className="flex items-center gap-1" title="reposts"><Repeat2 size={12} /> {post.repost_count}</span>
          )}
      ...
      <div className="font-mono text-sm text-gray-800 dark:text-gray-100 leading-relaxed mb-3 whitespace-pre-wrap">
        {post.content}
      </div>
        ...
        <div className="flex items-center gap-4 text-gray-400 dark:text-gray-500">
          ...
          {post.reply_count && post.reply_count > 0 && onViewReplies && (
            <button
              ...
              className="text-xs font-mono hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
            >
          ...
          {canDelete && (
            <button
              ...
              className="flex items-center gap-1 text-xs font-mono hover:text-red-600 dark:hover:text-red-400 transition-colors"
              aria-label="delete"
            >
      ...
      {showRepost && (
        <RepostModal
          postId={post.repost_of ? post.repost_of : post.id}
          onClose={() => setShowRepost(false)}
          onReposted={() => { setShowRepost(false); onReposted?.(); }}
        />
      )}
    </div>
  );
